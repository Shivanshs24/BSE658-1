...
ggplot(state_data, mapping=aes(x=Positive))  + geom_histogram(bins=10)
ggplot(state_data, mapping=aes(x=Positive))  + geom_histogram(bins=10, color="white", fill='steelblue')

```
```{r}
manystate_data= filter(covid_data, State== c('Delhi', 'Karnataka', 'Tamil Nadu', 'Uttar Pradesh'))
ggplot(manystate_data, mapping=aes(x= factor(State), y=Negative))  + geom_boxplot()
ggplot(manystate_data, mapping=aes(x= factor(State), y=Negative))  + geom_col()
ggplot(manystate_data, mapping=aes(x=Negative))  + geom_histogram(bins=10, color="white", fill='steelblue') + facet_wrap(~State, nrow=2)
```
